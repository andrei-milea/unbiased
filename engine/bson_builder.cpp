        ...
        [&article](array_context<> arr) {
            for (const auto& author : article.authors)
                arr << author;
                                ...
                                << close_array << "tf" << open_array << [&article](array_context<> arr) {
                                       for (auto elem : article.tf)
                                           arr << elem;
                                   }
                                << close_array << "keywords" << open_array << [&article](array_context<> arr) {
...
void from_bson(const bson_doc_view& doc, Vocabulary& vocab)
{
    ...
    for (const bsoncxx::array::element& word : words_array)
        vocab.stems_.insert(word.get_utf8().value.to_string());

    bsoncxx::array::view freq_array { doc["stems_freq"].get_array().value };
    size_t idx = 0;
    for (const bsoncxx::array::element& freq : freq_array)
    {
        vocab.stems_freq_[idx] = freq.get_int64().value;
        idx++;
    }
